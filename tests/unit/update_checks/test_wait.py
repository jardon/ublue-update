...
def test_transaction_success(mock_log, mock_run):
    ...
    mock_run.assert_called_once_with(["/usr/bin/rpm-ostree", "status", "--json"], capture_output=True)
...
def test_transaction_json_decode_error(mock_log, mock_run):
    ...
    mock_run.assert_called_once_with(["/usr/bin/rpm-ostree", "status", "--json"], capture_output=True)
    mock_log.error.assert_called_once_with("can't get transaction, system not managed with rpm-ostree container native")
