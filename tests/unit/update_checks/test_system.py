...
from ublue_update.update_checks.system import skopeo_inspect, system_update_check, pending_deployment_check
...
def test_skopeo_inspect(mock_run):
    ...
    mock_run.assert_called_once_with(["skopeo", "inspect", test_input], capture_output=True)
...
def test_system_update_check(mock_log, mock_skopeo_inspect, mock_run):
    ...
    mock_run.assert_called_once_with(["rpm-ostree", "status", "--json"], capture_output=True)
...
def test_system_update_check_json_error(mock_log, mock_run):
    ...
    assert not system_update_check()
    mock_log.error.assert_called_with("update check failed, system isn't managed by rpm-ostree container native")
    mock_run.assert_called_once_with(["rpm-ostree", "status", "--json"], capture_output=True)
...
def test_pending_deployment_check(mock_run):
    ...
    mock_run.assert_called_once_with(["rpm-ostree", "status", "--pending-exit-77"], capture_output=True)
